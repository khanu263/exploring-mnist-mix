...
def validation(data, label, model):
    ...
    torch_x_train = torch.from_numpy(data).type(torch.LongTensor)
    torch_y_train = torch.from_numpy(label).type(torch.LongTensor)  # data type is long
